...
- **v0.41.0** — Federation notification external alert delivery: alert delivery service with target matching and retry, Prometheus metrics export, JSONL structured export, retention/archive/purge service, metrics rollup, CLI commands, and console pages
- **v0.42.0** — Alert delivery productionization: retry scheduler, DLQ replay, alert deduplication, incremental rollup with checkpoints, HMAC-SHA256 webhook signing, archive cleanup, Phase 54 CLI commands and console pages
- **v0.45.0** — Alert delivery closed loop + production validation: daemon closed-loop integration with Phase 59 stores (idempotency, rate limiter, dead letter, distributed lock, key rotation), enhanced metrics recording, Prometheus endpoint, CLI run-once and metrics-prometheus commands, distributed lock leader election, key rotation auto-scheduling in daemon loop
...
- **v0.49.0** — Kubernetes and Deployment Hardening: hardened Docker image (non-root, HEALTHCHECK), Kubernetes manifests (Deployment, Service, ConfigMap, Secret, PVC, NetworkPolicy, PDB, validate-config Job), systemd unit templates, deployment config templates (example/K8s/systemd), smoke test scripts, deployment documentation and release checklist, 72 new unit tests
- **v0.48.0** — Persistent Approval / Control Plane: SQLite-backed control command state machine, persistent approval store (approve/reject/expire), persistent audit store, stdlib HTTP control server with Bearer token auth, daemon control polling (pause/resume/drain/shutdown/flush_metrics/release_lock/health_snapshot), health status extensions, CLI control commands (status/commands/send/get), 79 new unit tests
- **v0.47.0** — Daemon production operations hardening: graceful drain shutdown with inflight tracking, metrics ring buffer with periodic flush, lock lease extension for long-running batches, health HTTP server (/health, /ready), CLI daemon serve/health-server/drain commands, supervisor-friendly runtime behavior, 52 new unit tests
- **v0.46.0** — Daemon production runtime hardening: continuous daemon loop with leader/standby mode, lock renewal, start/stop lifecycle, health status, YAML config support, Prometheus file metrics exporter, async dead-letter evaluation, CLI daemon health/validate-config/metrics-export commands, 59 new unit tests
- **v0.44.0** — Multi-instance production readiness: DLQ replay idempotency, rate limiting, dead letter policy, enhanced metrics, webhook key rotation, distributed locks, console pages, and CLI commands for safe multi-instance notification delivery
- **v0.43.0** — Alert delivery operations chain: atomic priority queue with claim/ack/fail/requeue, retry daemon deep integration with priority queue, persistent daemon state store, webhook signing key rotation, SQLite concurrency safety with WAL mode, batch DLQ replay with confirmation, 6 new PolicyChangeEventType values, daemon health/state CLI commands
